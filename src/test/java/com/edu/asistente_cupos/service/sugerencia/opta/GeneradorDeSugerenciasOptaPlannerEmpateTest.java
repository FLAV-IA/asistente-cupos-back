...
@SpringBootTest
class GeneradorDeSugerenciasOptaPlannerEmpateTest {
  @Autowired
  ...
  void enEmpateDePrioridadSoloUnoEsAsignado() {
    ...
    List<SugerenciaInscripcion> asignadas = resultado.stream()
                                                     .filter(SugerenciaInscripcion::fueAceptada)
                                                     .toList();

    assertThat(asignadas).hasSize(1);
    SugerenciaAceptada aceptada = (SugerenciaAceptada) asignadas.get(0);
    assertThat(aceptada.comision().getCodigo()).isEqualTo("MAT2-A");

    List<SugerenciaInscripcion> rechazadas = resultado.stream().filter(s -> !s.fueAceptada())
                                                      .toList();
  ...
  void noSeSuperaElCupoDisponibleDeUnaComision() {
    ...
    List<SugerenciaInscripcion> sugerencias = asignador.sugerir(
    ...
    List<SugerenciaInscripcion> rechazadas = sugerencias.stream().filter(s -> !s.fueAceptada())
                                                        .toList();

    assertThat(sugerencias).hasSize(2);
    assertThat(asignadas).hasSizeLessThanOrEqualTo(unicaComision.getCupo());
    assertThat(asignadas).extracting(s -> ((SugerenciaAceptada) s).comision().getCodigo())
                         .containsOnly("MAT2-A");
